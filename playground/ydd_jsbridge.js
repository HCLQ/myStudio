...
!(function() {
  if (!window._dsf) {
        ...
        if (syncCallback) {
          ...
          if (2 <= syncCallback.length) {
            ...
            if (callback && 'function' == typeof callback) {
              ...
              if ((callback = asyncCallback[payload]) && 'function' == typeof callback) {
    ...
    let key
    for (key in assignToWin) window[key] = assignToWin[key]
    bridge.register('_hasJavascriptMethod', function(names) {
      let nameArray = names.split('.')
      if (2 > nameArray.length) return !(!_dsf[nameArray] && !_dsaf[nameArray])
      names = nameArray.pop()
      nameArray = nameArray.join('.')
      return (nameArray = _dsf._obs[nameArray] || _dsaf._obs[nameArray]) && !!nameArray[names]
    })
  }
})()
